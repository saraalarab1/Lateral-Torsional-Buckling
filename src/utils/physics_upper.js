    ...
    for (let i = 0; i < positions_upper.length / 3; i++) {
        ...
        if ((params.left === 'Pin') && (params.right === 'Pin')) {
            ...
            if (x < a) {
                ...
                bending_moment_upper.push(R1 * x);
                shear_force_upper.push(R1);
            } else {
                deflection = P * a * ((l - x) ** 2 * (3 * l * l * x - a * a * x - 2 * a * a * l)) / (12 * EI * l * l * l);
                bending_moment_upper.push(R1 * x - P * (x - a));
                shear_force_upper.push(-R2);
            }
        } else if ((params.left === 'Fixed') && (params.right === 'Pin')) {
            // IMPLEMENTED AND PROBABLY WORKING
            ...
            if (x > a) {
                ...
                bending_moment_upper.push(R2 * (l - x));
                shear_force_upper.push(-R2);
            } else {
                deflection = P * b * (x ** 2 * (3 * l * l * (l - x) - b * b * (l - x) - 2 * b * b * l)) / (12 * EI * l * l * l);
                bending_moment_upper.push(R2 * (l - x) - P * ((l - x) - b));
                shear_force_upper.push(R2);
            }
        } else if ((params.left === 'Fixed') && (params.right === 'Free')) {
            // IMPLEMENTED AND PROBABLY WORKING
            if (x > a) {
                ...
                bending_moment_upper.push(0);
                shear_force_upper.push(0);
            } else {
                deflection = P * x ** 2 * (3 * a - x) / (6 * EI);
                bending_moment_upper.push(P * (l - x - a));
                shear_force_upper.push(-P);
            }
        } else if ((params.left === 'Free') && (params.right === 'Fixed')) {
            // IMPLEMENTED AND PROBABLY WORKING
            if (x < a) {
                ...
                bending_moment_upper.push(P * (x - a));
                shear_force_upper.push(-P);
            }
        } else {
            deflection = 0;
